    ...
    <>
      ...
      <div className="px-2 py-2  -slate-100 ">
        { basket?.map((book:Book) => (
          <div
            key={book.id}
            className="grid grid-cols-[1fr,3fr] bg-slate-500  my-3  gap-2 shadow-lg rounded-b-2xl"
          >
            <div className="   h-[150px] justify-center items-center    ">
              <img
                src={book.image}
                alt={book.title}
                className="flex top-1 bg-black object-contain w-full h-full p-3"
              />
            </div>
          </div>
        ))}
