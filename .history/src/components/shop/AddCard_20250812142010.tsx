...
import { Book, typefilebasket } from "../Data/interfaceDATA";
...
type Props = {
  book: Book;
    databook:typefilebasket

};

export default function Addcard({ book ,databook }: Props) {
  ...
  >({
    mutationFn: (deletecart: TypeDeletecartItem) => {
      return axioshandel.delete(
        `/basket/Single/${deletecart?.order_id}/${deletecart?.databook.file.id}`
      );
    },
    onSuccess: () => {
