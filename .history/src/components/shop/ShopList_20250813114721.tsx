...
console.log(basket);
    ...
    const mathtest = basket?.data.flatMap((item)=>item.book_files.map((fill)=>({
      price:fill.price , 
      status:fill.status ,
    }))
  )  
  const MathpriceItem = basket?.map((item)=>item.price) 

  console.log(mathtest,"math");
    


  if (mathtest.status===2) {
    MathpriceItem = mathtest.price+MathpriceItem
  }
