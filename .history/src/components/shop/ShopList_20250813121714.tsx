...
export default function Shop() {
  const [cartExtraPrices, setCartExtraPrices] = useState<{ [bookId: string]: number }>({});
  ...
  useEffect(() => {
    if (!basket) return;
    const totalPrice = basket.data.reduce((acc, book) => {
      const extra = cartExtraPrices[book.id] ?? 0;
      return acc + book.price + extra;
    }, 0);
    setTotal(totalPrice);
  }, [basket, cartExtraPrices]);

  // تابع اضافه کردن قیمت فایل اضافی (خلاصه یا ترجمه)
  const addExtraPrice = (bookId: string, priceToAdd: number) => {
    setCartExtraPrices(prev => ({
      ...prev,
      [bookId]: (prev[bookId] ?? 0) + priceToAdd
    }));
  };
    ...
    <>
      ...
      <div className="px-2 py-2 -slate-100 ">
        {basket?.data.map((book: Book) => {
          const extraPrice = cartExtraPrices[book.id] ?? 0;
                ...
                <div className="flex flex-col w-full items-center">
                  ...
                  <div className="flex justify-between w-full mt-2">
                    ...
                    <button
                      className="flex items-center"
                      onClick={() => {
                        const price = book.book_files.find(f => f.status === 1)?.price ?? 0;
                        addExtraPrice(book.id, price);
                      }}
                    >
                      <IoIosAdd style={{ color: "#DD7475" }} />
