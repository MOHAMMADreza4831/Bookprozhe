...
export default function Addcard({ book }: Props) {
  ...
  const addcartMutation = useMutation<typeDeletecartItem, Error, datasubmit>({
  ...
  const DeletecartItem = useMutation<deletecart, Error, deletecart>({
    mutationFn: (deletecart: deletecart) => {
      return axioshandel.post("/basket/Single/{basket}/{file}", deletecart);
    },
