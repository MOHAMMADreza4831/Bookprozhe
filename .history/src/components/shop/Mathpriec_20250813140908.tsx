...
export default function Mathprice () {
  ...
  const handleAddTranslation = () => {
    if (!translationAdded) {
      setExtraPrice(prev => prev + (book.book_files.find(f => f.status === 1)?.price ?? 0));
      console.log(translationAdded,"math1");
      
      setTranslationAdded(true);
    }
  };
  const handleAddSSummary = () => {
    if (!summaryAdded) {
      setSummaryAdded(prev => prev + (book.book_files.find(f => f.status === 2)?.price ?? 0));
      console.log(summaryAdded,"math2");
