...
export default function Addcard({ book }: Props) {
  ...
  >({
    mutationFn: (deletecart: TypeDeletecartItem) => {
      //  deletecart?.find((item) => item.id === book.book_id);
      return axioshandel.delete(
        `/basket/Single/${deletecart?.order_id}/${deletecart?.id}`
      );
    },
    onSuccess: () => {
  ...
  const handelsubmit = async (itemid:number) => {
    // const exist = basket?.some((item) => item.book_id === book.id);
  ...
  return (
    ...
    >
      <button
        onClick={() => handelsubmit(item.order_id)}
